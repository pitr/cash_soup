var OANDA = OANDA || {};
OANDA.baseURL = OANDA.baseURL || "http://graupel.oanda.com:8081";
OANDA.apiKey = "12345";
...
OANDA.rate.list_symbols = function(sessionToken, callback) {
    $.getJSON(OANDA.baseURL + "/v1/rate/list_symbols.json?callback=?", {session_token: sessionToken}, function(response, textStatus) {
        if(callback) {
            callback(response);
        }
    });
...
OANDA.rate.quote = function(sessionToken, symbols, callback) {
    ...
    $.getJSON(OANDA.baseURL + "/v1/rate/quote.json?callback=?", {symbols: symbolStr, session_token: sessionToken}, function(response, textStatus) {
        if(callback) {
            callback(response);
        }
    });
